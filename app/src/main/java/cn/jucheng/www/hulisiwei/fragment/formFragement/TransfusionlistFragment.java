...
import cn.jucheng.www.hulisiwei.adapter.fragmentAdapter.TransfusionfragmentAdapter;
...
public class TransfusionlistFragment extends BaseFragment implements TransfusionfragmentAdapter.ToastListener,AbsListView.OnScrollListener {

    @BindView(R.id.fragment_fitlist)
    MyList fragmentTransfuisonList;
    ...
    TransfusionfragmentAdapter adapter;
    ...
    public void initAdapter() {
        adapter = new TransfusionfragmentAdapter(getActivity(), UserMessage.fragmentHead, pages);
        fragmentTransfuisonList.setAdapter(adapter);
        adapter.setOntoastClickListener(this);
        fragmentTransfuisonList.setOnScrollListener(this);
    }
    ...
    public void setBiaodan(String message){
        ...
        if(biaoDanType == 18){
            ...
            if(UserMessage.transfusion_Message.size() >= yiZhuHangHaos){
                specailList = new ArrayList<>();
                specailList = CommUtils.getJson(messaegs, "shuyedan");
                specailList.add(yiZhuType);
                specailList.add(yiZhuHangHao);
                UserMessage.transfusion_Message.set(hang,specailList);
            }else {
                specailList = new ArrayList<>();
                specailList = CommUtils.getJson(messaegs, "shuyedan");
                specailList.add(yiZhuType);
                specailList.add(yiZhuHangHao);
                UserMessage.transfusion_Message.add(specailList);
            }
        }
    }
