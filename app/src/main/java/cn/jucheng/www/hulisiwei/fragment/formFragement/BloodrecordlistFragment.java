...
import cn.jucheng.www.hulisiwei.adapter.fragmentAdapter.BloodrecordfragmentAdapter;
...
public class BloodrecordlistFragment extends BaseFragment implements BloodrecordfragmentAdapter.ToastListener,AbsListView.OnScrollListener{


    @BindView(R.id.fragment_fitlist)
    MyList fragmentBloodrecordlist;
    ...
    BloodrecordfragmentAdapter adapter;
    ...
    public void initAdapter() {
        adapter = new BloodrecordfragmentAdapter(getActivity(), UserMessage.fragmentHead, pages);
        fragmentBloodrecordlist.setAdapter(adapter);
        adapter.setOntoastClickListener(this);
        fragmentBloodrecordlist.setOnScrollListener(this);
    }
     ...
     */
    public void getPage() {
        int a = UserMessage.bloodrecord_Message.size() / 5;
        if (UserMessage.bloodrecord_Message.size() > 5) {
            pages = pages + a;
        }
    }
            ...
            case 3://3是签字信息
                ...
                switch (qianzi_zhuangTai){
                    case 1://签字成功
                        ...
                        list = UserMessage.bloodrecord_Message.get(hang);
                        list.set(0,date);
                        list.set(25,nurseName);
                        list.set(26, String.valueOf(qianzi_yizhuzhonglei));
                        break;
                    case 2://失败
    ...
    public void setBiaodan(String message){
        ...
        if (biaoDanType == 20) {
            ...
            if(UserMessage.bloodrecord_Message.size() >= yiZhuHangHaos){
                specailList = new ArrayList<>();
                specailList = CommUtils.getJson(messaegs, "shuxueguanchajiludan");
                specailList.add(yiZhuType);
                specailList.add(yiZhuHangHao);
                UserMessage.bloodrecord_Message.set(hang,specailList);
            }else {
                specailList = new ArrayList<>();
                specailList = CommUtils.getJson(messaegs, "shuxueguanchajiludan");
                specailList.add(yiZhuType);
                specailList.add(yiZhuHangHao);
                UserMessage.bloodrecord_Message.add(specailList);
            }
        }
    }
    ...
    public void ClearBiaodanHead() {
        ...
        UserMessage.bloodrecord_Message.clear();
        pages = 1;
        adapter.setLists(UserMessage.fragmentHead, pages);
    }
