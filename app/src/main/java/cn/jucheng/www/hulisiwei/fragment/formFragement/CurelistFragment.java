...
import cn.jucheng.www.hulisiwei.adapter.fragmentAdapter.CurefragmentAdapter;
...
public class CurelistFragment extends BaseFragment implements CurefragmentAdapter.ToastListener,AbsListView.OnScrollListener {


    @BindView(R.id.fragment_fitlist)
    MyList fragmentCurelist;

    Unbinder unbinder;
    private View view;

    List<String> specailList = new ArrayList<>();
    CurefragmentAdapter curefragmentAdapter;
    ...
    public void initAdapter() {
        curefragmentAdapter = new CurefragmentAdapter(getActivity(), UserMessage.fragmentHead, pages);
        fragmentCurelist.setAdapter(curefragmentAdapter);
        curefragmentAdapter.setOntoastClickListener(this);
        fragmentCurelist.setOnScrollListener(this);
    }
    ...
    public void onEventMainThread(MessageEvent evnt) {
        ...
        if (biaoDanType == 16) {
            pages = 1;
        getPage();
        curefragmentAdapter.setLists(UserMessage.fragmentHead, pages);
        }
    }
     ...
     */
    public void getPage() {
        int a = UserMessage.cure_Message.size() / 22;
        if (UserMessage.cure_Message.size() > 22) {
            pages = pages + a;
        }
    }
            ...
            case 3://3是签字信息
                ...
                switch (qianzi_zhuangTai){
                    case 1:
                        ...
                        list = UserMessage.cure_Message.get(hang);
                        list.set(0,date);
                        list.set(3,nurseName);
                        list.set(4, String.valueOf(qianzi_yizhuzhonglei));
                        break;
                    case 2:
    ...
    public void setBiaodan(String message){
        ...
        if(biaoDanType == 16){
            ...
            if(UserMessage.cure_Message.size() >= yiZhuHangHaos){
                specailList = new ArrayList<>();
                specailList = CommUtils.getJson(messaegs, "zhiliaodan");
                specailList.add(yiZhuType);
                specailList.add(yiZhuHangHao);
                UserMessage.cure_Message.set(hang,specailList);
            }else {
                specailList = new ArrayList<>();
                specailList = CommUtils.getJson(messaegs, "zhiliaodan");
                specailList.add(yiZhuType);
                specailList.add(yiZhuHangHao);
                UserMessage.cure_Message.add(specailList);
            }
        }
    }
    ...
    public void ClearBiaodanHead() {
        ...
        UserMessage.cure_Message.clear();
        pages = 1;
        curefragmentAdapter.setLists(UserMessage.fragmentHead, pages);
    }
