...
import cn.jucheng.www.hulisiwei.adapter.fragmentAdapter.TransfusioncordFragmentAdapter;
...
public class TransfusionrecordFragment extends BaseFragment implements TransfusioncordFragmentAdapter.ToastListener,AbsListView.OnScrollListener {
    ...
    MyList fragmentTrancordlist;

    /**
     * 数据源
     */
    List<String> specailList = new ArrayList<>();
    TransfusioncordFragmentAdapter adapter;
    ...
    public void initAdapter() {
        adapter = new TransfusioncordFragmentAdapter(getActivity(), UserMessage.fragmentHead, pages);
        fragmentTrancordlist.setAdapter(adapter);
        adapter.setOntoastClickListener(this);
        fragmentTrancordlist.setOnScrollListener(this);
    }
     ...
     */
    public void getPage() {
        int a = UserMessage.transfusincord_Message.size() / 31;
        if (UserMessage.transfusincord_Message.size() > 31) {
            pages = pages + a;
        }
    }
            ...
            case 3://3是签字信息
                ...
                switch (qianzi_zhuangTai){
                    case 1:
                        ...
                        list = UserMessage.transfusincord_Message.get(hang);
                        list.set(4,date);
                        list.set(5,nurseName);
                        list.set(6, String.valueOf(qianzi_yizhuzhonglei));
                        break;
                    case 2:
    ...
    public void setBiaodan(String message){
        ...
        if(biaoDanType == 21){
            ...
            if(UserMessage.transfusincord_Message.size() >= yiZhuHangHaos){
                specailList = new ArrayList<>();
                specailList = CommUtils.getJson(messaegs, "shuyejiludan");
                specailList.add(yiZhuType);
                specailList.add(yiZhuHangHao);
                UserMessage.transfusincord_Message.set(hang,specailList);
            }else {
                specailList = new ArrayList<>();
                specailList = CommUtils.getJson(messaegs, "shuyejiludan");
                specailList.add(yiZhuType);
                specailList.add(yiZhuHangHao);
                UserMessage.transfusincord_Message.add(specailList);
            }
        }
    }
    ...
    public void ClearBiaodanHead() {
        ...
        UserMessage.transfusincord_Message.clear();
        pages = 1;
        adapter.setLists(UserMessage.fragmentHead, pages);
    }
