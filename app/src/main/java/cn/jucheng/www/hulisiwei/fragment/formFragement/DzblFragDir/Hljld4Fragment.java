...
import cn.jucheng.www.hulisiwei.adapter.fragmentAdapter.HLJL4FragmentAdapter;
...
public class Hljld4Fragment extends BaseFragment implements AbsListView.OnScrollListener {
    ...
    public void initAdapter() {
        adapter = new HLJL4FragmentAdapter(getActivity(), UserMessage.fragmentHead, pages);
    ...
    public void setBiaodan(String message){
        ...
        int colnum =Integer.parseInt(SubStringUtils.substring(message,56,60),16);
        String json = HexadecimalConver.decode(SubStringUtils.substring(message,60,64+(validLenth-4)*2));
        List<String> specialList = CommUtils.getJson(json, "yachuanghulijiludan");
        if(UserMessage.YchljlDan.size()==colnum){
            UserMessage.YchljlDan.set(colnum,specialList);
        }else{
            UserMessage.YchljlDan.add(specialList);
        }
        }
